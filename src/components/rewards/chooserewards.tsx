...
const ChooseReward = () => {
    ...
    // TODO: not actively being worked on, integrate database first
    const {activeUser} = useContext(UserContext);
    Promise.resolve(getUserDatabase()).then((value) => {activeUser = value})
    .then(() => {
        if(
            ...
            ((new URLSearchParams(window.location.search)).get('ref') === "challenger"
            &&
            activeUser.badges.length === 8 && 
            activeUser.challengeReward !== undefined) ||
            
            (((new URLSearchParams(window.location.search)).get('ref') === "player")
            && activeUser.playerReward !== undefined) ){
